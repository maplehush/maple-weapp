...
test('published package contains its declared npm entry file', () => {
  ...
  assert.ok(
    packedFiles.has(packageJson.main),
    `published package must include its main entry: ${packageJson.main}`
  )
})
...
test('example npm references resolve inside the published miniprogram root', () => {
  const packageJson = JSON.parse(readFileSync(resolve('package.json'), 'utf8'))
  assert.equal(
    typeof packageJson.miniprogram,
    'string',
    'package.json must declare a miniprogram source root'
  )
  ...
  for (const exampleFile of exampleFiles) {
    ...
    for (const [, reference] of references) {
      ...
      assert.ok(
        candidates.every(existsSync),
        `${packagePrefix}${reference} must resolve to published miniprogram source files`
      )
    }
  }
})
