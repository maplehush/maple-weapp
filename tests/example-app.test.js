...
test('example app exposes only the component catalog flow', () => {
  const appConfig = JSON.parse(
    readFileSync(resolve('examples/miniprogram/miniprogram/app.json'), 'utf8')
  )
  const indexConfig = JSON.parse(
    readFileSync(resolve('examples/miniprogram/miniprogram/pages/index/index.json'), 'utf8')
  )
  const detailConfig = JSON.parse(
    readFileSync(resolve('examples/miniprogram/miniprogram/pages/detail/index.json'), 'utf8')
  )
  ...
  for (const page of appConfig.pages) {
    for (const extension of ['.json', '.ts', '.wxml', '.wxss']) {
      assert.ok(
        existsSync(resolve('examples/miniprogram/miniprogram', page + extension)),
        `${page}${extension} must exist`
      )
    }
  }
})
