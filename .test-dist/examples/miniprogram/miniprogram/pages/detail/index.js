...
const componentDocs = {
    ...
    icon: { name: 'Icon', title: '图标', description: '内置常用图标字体，也支持网络地址与本地图片路径。', props: ['name: string', 'size: number', 'color: string', 'mode: string'], usage: '<m-icon name="check" size="40" color="#07c160" />' },
    ...
    navbar: { name: 'Navbar', title: '导航栏', description: '适配顶部安全区，提供标题、返回和左右插槽。', props: ['title: string', 'backable: boolean', 'fixed: boolean', 'border: boolean', 'safeArea: boolean'], usage: '<m-navbar title="页面标题" backable fixed />' },
...
Component({
    data: {
        ...
        iconColors: [
            { label: '品牌色', value: 'var(--m-color-primary)' },
            { label: '成功', value: 'var(--m-color-success)' },
            { label: '警告', value: 'var(--m-color-warning)' },
            { label: '危险', value: 'var(--m-color-danger)' }
        ],
        imageIconUrl: '/assets/icon-demo.svg'
    },
    methods: {
        onLoad(options) {
            const componentId = options.id || '';
            const doc = componentId ? componentDocs[componentId] : null;
            this.setData({ componentId, doc });
            if (doc)
                wx.setNavigationBarTitle({ title: doc.name });
        },
        onBack() {
            wx.navigateBack();
        }
